...
# Import OCSVM trainer from the project's model folder.
ocsvm_dir = os.path.join(parent_dir, 'Models', 'anomaly_detection', 'ocsvm')
...
def main():
    ...
    try:
        # Validate and load the required CICFlowMeter CSV files before the
        # training/detection pipeline receives any data.
        df_full = DatasetLoader.validate_and_load_cicflowmeter_datasets(label_column='Label')

        # Optionally sample the combined dataset for faster local training.
        if CONFIG['sample_size'] and len(df_full) > CONFIG['sample_size']:
            df_train = df_full.sample(n=CONFIG['sample_size'], random_state=42)
            print(f"Using sampled training dataset: {df_train.shape}")
        else:
            df_train = df_full
        ...
        print("Please fix the dataset paths before running the main pipeline.")
        return
